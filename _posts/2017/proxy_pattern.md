...
- 上面代理的实现中，new一个代理实例的handler参数使用了匿名内部类，具体参照匿名内部类。
  - 匿名内部类所做的是创建了一个 InvocationHandler 接口的实例，这个实例的 invoke() 方法是用代理目标作为对象（将代理方法分发到这个代理对象上去）；
  - Proxy.newProxyInstance()方法中指定了目标类的类加载器，目标执行的接口，方法调用时所用的方法调用处理器；

**问题：**

1. 在这里 Proxy.newProxyInstance() 方法中，最后一个参数 InvocationHandler 的 invoke 方法的第一个参数是 proxy 本身，对于 InvocationHandler 接口应用在此处不当：接口第一个参数是需要 代理的实例，而外层调用却是 创建该代理实现，明显接口的第一个参数多余的（仅在于此处，其他地方 handler 可要需要使用到代理实例）。
